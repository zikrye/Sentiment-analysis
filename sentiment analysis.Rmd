...
The book id for _THE RED AND THE BLACK_ is 44747 on [gutenberg website](http://aleph.gutenberg.org.). The original dataframe contains the id and text of the book in unorganized form. After using function unnest_tokens and filtering out the words that do not have real meanings from stop_word database, each observation only include a meaningful word extracted from the book. Plus, the line number and the chapter the words belong to are also included. Analysis of sentiment uses [Text mining in R](https://www.tidytextmining.com/index.html) as reference.
